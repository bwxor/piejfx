...
- [Building binaries](#building-binaries)
  - [Building using the provided scripts](#building-using-the-provided-scripts)
  - [Building manually](#building-manually)
    - [Build the piejfx-plugin-core project first, then add it to the local Maven repository](#build-the-piejfx-plugin-core-project-first-then-add-it-to-the-local-maven-repository)
    - [Build the piejfx project](#build-the-piejfx-project)
    - [Move the jar (together with its dependencies) in a separate folder](#move-the-jar-together-with-its-dependencies-in-a-separate-folder)
    - [Use jpackage to create a binary file](#use-jpackage-to-create-a-binary-file)
      - [For Windows](#for-windows)
      - [For Linux](#for-linux)
- [Extending functionality by developing custom plugins](#extending-functionality-by-developing-custom-plugins)
  - [Introduction](#introduction)
  - [Hooks and the Plugin interface](#hooks-and-the-plugin-interface)
    - [The onLoad hook](#the-onload-hook)
  - [Creating a custom UI for your plugin](#creating-a-custom-ui-for-your-plugin)
  - [Where do I store the plugin data?](#where-do-i-store-the-plugin-data)
    - [Storing the plugin artifact](#storing-the-plugin-artifact)
    - [Storing the dependencies](#storing-the-dependencies)
...
#### Build the `piejfx-plugin-core` project first, then add it to the local Maven repository
This can be done by running the `mvn install` inside the `piejfx-plugin-core/` folder.

#### Build the `piejfx` project
You can do this through the `mvn package` command (or directly from IntelliJ's Maven interface).

#### Move the jar (together with its dependencies) in a separate folder
...
#### Use `jpackage` to create a binary file
...
Dependencies should be stored in a separate `deps/` folder, which is located under the same level as the plugin jar. The `deps/` folder should containg everything that's included as a Maven dependency inside your plugin's `pom.xml` file. Such a dependency folder can be generated by simply running

```
mvn dependency:copy-dependencies -DoutputDirectory=<DIRECTORY_NAME>
```
...
1. [piejfx-maven-plugin](https://github.com/bwxor/piejfx-maven-plugin)
2. [piejfx-git-plugin](https://github.com/bwxor/piejfx-git-plugin)
3. [piejfx-documentation-plugin](https://github.com/bwxor/piejfx-documentation-plugin)
4. [crumb](https://github.com/bwxor/crumb)
